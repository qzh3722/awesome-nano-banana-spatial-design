...
增强室内渲染图的光照品质：强化灯具光晕、恢复暗部细节、优化光线衰减、提升空间层次。非常适合渲染图后期优化。
...
光影增强：
1. 吊灯光晕：强化灯具散发的暖色光晕。在每个光源周围添加柔和的光晕和自然的光线扩散效果。

2. 暗部恢复：提亮阴影区域（桌下、角落），显现隐藏的细节，同时保持画面深度，避免平淡或过度曝光。

3. 光线衰减：应用真实的平方反比光线衰减规律。靠近光源的区域应明显更亮，并呈现平滑的渐暗过渡。

4. 天花反光：在天花板表面添加来自下方灯具和环境光的微妙暖色反射。

5. 层次增强：在前景（更亮）和背景区域（稍暗）之间创建清晰的光照层次分离，增强空间纵深感。
...
> **💡 提示**：如果效果过于戏剧化，添加"轻柔"或"微妙"关键词。使用"戏剧性光晕效果"可增强光晕。
