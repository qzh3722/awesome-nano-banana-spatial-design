...
**本阶段案例：**  
[3.1 平面图转轴测图](#31-平面图转轴测图-plan-to-isometric) • [3.2 衣柜内部结构可视化](#32-衣柜内部结构可视化-wardrobe-interior) • [3.3 立面图转渲染](#33-立面图转渲染-elevation-to-render)
...
### 3.3 立面图转渲染 (Elevation to Render)
将2D技术立面图纸转换为照片级材质可视化效果。

**提供两种方法：**
1.  **通用模板 (Universal Template)**：适用于简单的标准案例。
2.  **元提示词生成器 (Meta-Prompt Generator)**：适用于带标注的复杂图纸。让 AI 帮你读取 CAD 标注并撰写定制提示词。
...
**本阶段案例：**  
[5.1 白模转照片级渲染](#51-白模转照片级渲染-clay-to-photorealistic) • [5.2 光影修复与强化](#52-光影修复与强化-lighting-enhancement) • [5.3 色调统一与色彩校正](#53-洗图2色调统一与色彩校正) • [5.4 日景转夜景](#54-日景转夜景)
...
将任意日间渲染图（室内或室外）智能转换为写实夜景，自动激活灯光。
...
> **💡 提示**：此通用提示词适用于室内和室外场景，AI 会自动判断场景类型并应用合适的夜间照明。
